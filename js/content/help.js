...
GMB.content = {
  ...
  pages: {
    scenario: { intro: "Define malaria interventions.",
      ...
      tips: ["Hover the info icons for additional information.", "All changes are unsaved until you press Save; use Discard to undo unsaved changes."] },
    ...
    costs: { intro: "Define unit costs assumptions.",
      ...
      tips: ["Add or remove cost lines with the + and x controls."] },
    ...
    generate: { intro: "Combine scenario and cost information to create a complete budget.",
      steps: ["Choose a scenario and a cost set.",
        ...
        "Or add several scenario x cost-set combinations to the queue and generate them together.",
  ...
  methods: {
    ...
    interventions: [
      ...
      { code: "mii_routine", name: "Routine / continuous ITN", target: "ANC & infant contacts",
        quant: "Nets needed  =  Routine-Eligible Population x Coverage x (1 + Buffer)",
      ...
      { code: "irs", name: "Indoor residual spraying (IRS)", target: "Households in sprayed areas",
        quant: "Structures to be sprayed =  Households x Coverage x (1 + Buffer)",
      ...
      { code: "smc", name: "Seasonal malaria chemoprevention (SMC)", target: "Children 3-59 months",
        quant: "SPAQ Packs = Age-Band Eligible Children x Coverage x Cycles x (1 + Buffer)",
      ...
      { code: "iptsc", name: "IPT for school-age children", target: "School-age children (5-15 years)",
        quant: "DHA-PPQ Packs = Age-Band Eligible Children x Coverage x Cycles x (1 + Buffer)",
      ...
      { code: "vax", name: "Malaria vaccine", target: "Eligible infant cohort",
        quant: "Vaccine Doses  =  Infant Cohort x (Sum of Dose Coverages) x (1 + Buffer)",
    ...
    sections: {
      costing: { title: "How costs are applied",
        ...
        blocks: [{ formula: "Line Item Cost:  Quantity Used For Cost x Unit Price" }, { formula: "Commodity Quantity:  Target Population x Coverage x Rounds or Doses x (1 + Buffer)" }, { formula: "Fixed Line (per year / one-off):  Applicable Occurrences x Unit Price" },
